...
describe.skipIf(!url)("aggregate repository PostgreSQL invariants", () => {
  ...
  it("replaces inactive rows deterministically and leaves active rows protected", async () => {
    ...
    await repository.preparePublication(publicationId);
    await repository.flushGroup(publicationId, { championId: 1, role: "TOP", baseline: { wins: 1, losses: 0, sample: 1 }, items: new Map([[3031, { wins: 1, losses: 0, sample: 1 }]]), pairs: new Map(), trios: new Map(), boots: new Map() });
    expect((await repository.rows(publicationId)).length).toBe(2);
    await expect(repository.preparePublication(publicationId)).resolves.toBeUndefined();
  });
});
