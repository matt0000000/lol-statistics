...
export function validateSeedEnvironment(env: Record<string, string | undefined> = process.env): URL {
  ...
  if (!databaseName.endsWith("_test")) throw new Error("seed:e2e refuses databases whose name does not end in _test");
  return url;
}
  ...
  try {
    await sql.begin(async (tx) => {
      ...
      await tx.unsafe(`INSERT INTO patches (id, version, patch_key, activated_at, published_at, is_active)
        VALUES (1, '${PATCH_VERSION}', '16.15', '2026-08-01T00:00:00Z', '2026-08-01T12:00:00Z', true)`);
      await tx.unsafe(`INSERT INTO collection_runs
        ...
        VALUES ('${RUN_ID}', 'COMPLETED', 'publish', 1, 35, 100, '2026-08-01T00:00:00Z', '2026-08-01T12:00:00Z',
          200, 200, 150, 0, '${PUBLICATION_ID}', '2026-08-01T12:00:00Z')`);
      await tx.unsafe(`INSERT INTO aggregate_publications
        ...
        VALUES ('${PUBLICATION_ID}', 1, '${RUN_ID}', '2026-07-01T00:00:00Z', '2026-08-01T12:00:00Z', 100, true)`);
