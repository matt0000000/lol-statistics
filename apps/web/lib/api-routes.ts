...
import { cachedJson, contentEtag, publicationEtag } from "./http-cache";
...
export function createRouteHandlers(queries: RouteQueries, dependencies: { scopeOf?: (value: unknown) => string | undefined } = {}) {
  ...
  const meta: Handler = async (request) => {
    ...
    const id = scopeOf(result.value);
    if (!id) return responseForError({ code: "internal_error" });
    return cachedJson(request, value.data, publicationEtag(id, "meta"));
  };
  ...
  const champions: Handler = async (request) => {
    ...
    const scopeId = scopeOf(result.value);
  ...
  const champion: Handler = async (request, context) => {
    ...
    const scopeId = scopeOf(result.value); if (!scopeId) return responseForError({ code: "internal_error" });
  ...
  const stats: Handler = async (request, context) => {
    ...
    if (includeRaw !== undefined && !["true", "false"].includes(includeRaw)) return invalid();
    const includeLowConfidence = includeRaw === "true";
  ...
  const methodology: Handler = async (request) => {
    ...
    return cachedJson(request, output.data, await contentEtag(output.data));
  };
